...
sys_symlink(void){
  ...
  if((dp = nameiparent(path, name)) == 0)
    goto bad;
  ilock(dp);
  if((pp = create(path, T_SYMLINK, 0, 0)) == 0){
    iunlockput(dp);
    goto bad;
  }
  iunlockput(dp);
  ...
  ilock(pp); 
  if(writei(pp, 0, (uint64)target, pp->size, MAXPATH)==0){
    iunlockput(pp);
    goto bad;
  }
  pp->type = T_SYMLINK;
...
nameii(char *path, uint depth, int omode){
  ...
  if((ip->type==T_SYMLINK) && !(omode & O_NOFOLLOW)){
    ...
    if(readi(ip, 0, (uint64)link_path, ip->size-MAXPATH, MAXPATH)==0){
      iunlockput(ip);
      return 0;
    } 
    iunlockput(ip);
